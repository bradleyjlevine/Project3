 ...
 * Summary: Create 80 cubes and 3 sphers (that are point lights) with movement all enclosed in a skybox.
 * Files: Program.cs (main), Game1.cs (game code), GeometricPrimitives.cs (provided), SpherePrimitives.cs (provided), VertexPositionNormal.cs (provided),
 * Cube.cs (made my own version), pointlight.fx, ambient.fx (used for testing), skybox.fx (provided), SkyBoxTexture.dds (texture cube provided)
 * Issues: Only two of the point lights will actually shade.  I would get this weird error with the pointlight.fx file when trying to do three.  Error: "Compiled shader code uses too many arithmetic instruction slots (72)." 
 * The pixel shader complier was targeting ps_2_0 for some reason even though at the top it is defined as "#define PS_SHADERMODEL ps_4_0_level_9_1".  Well my graphics card has Shader Model 2.0 :(
 */
